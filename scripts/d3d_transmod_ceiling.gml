/*
**  Usage:
**      d3d_transmod_ceiling(ind, x1,y1,z1, x2,y2,z2, hrep,vrep,
...
**      The sloping is in the direction of the x-axis, just as with
**      d3d_model_floor.
...
*/
{
    ...
    nx = (y1 - y2) * (z2 - z1);
    nz = (x2 - x1) * (y2 - y1);
    n = 1 / sqrt (sqr(nx) + sqr(nz));
    nx *= n;
    nz *= n;
    d3d_transmod_primitive_begin (ind, pr_trianglefan);
    d3d_transmod_vertex_normal_texture (ind, x1, y1, z1, nx, 0, nz,
                                        hoffset, voffset, ns);
    d3d_transmod_vertex_normal_texture (ind, x1, y2, z1, nx, 0, nz,
                                        hoffset, voffset + vrep, ns);
    d3d_transmod_vertex_normal_texture (ind, x2, y2, z2, nx, 0, nz, 
                                        hoffset + hrep, voffset + vrep, ns);
    d3d_transmod_vertex_normal_texture (ind, x2, y1, z2, nx, 0, nz, 
